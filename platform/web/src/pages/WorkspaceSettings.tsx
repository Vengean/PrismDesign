...
export default function WorkspaceSettings() {
  ...
  const [agentType, setAgentType] = useState<"claude" | "glm" | "claude-sub">("claude");
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((ws) => {
        ...
        setGitSshPort(ws.git_ssh_port || 22);
        setAuthMode(ws.git_ssh_key ? "ssh" : "token");
        setAgentType((ws.agent_type as "claude" | "glm" | "claude-sub") || "claude");
        setAnthropicApiKey(ws.anthropic_api_key || "");
        setAnthropicBaseUrl(ws.anthropic_base_url || "");
        setAnthropicModel(ws.anthropic_model || "");
        setHttpsProxy(ws.https_proxy || "");
        setRepos(ws.repos.length > 0 ? ws.repos : [emptyRepo()]);
        setClaudeMd(ws.claude_md);
        setStartupScript(ws.startup_script);
      })
      .catch(() => setError("工作空间不存在"))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const isSub = agentType === "claude-sub";
      await api.updateWorkspace(id!, {
        ...
        gitSshPort,
        agentType,
        anthropicApiKey: isSub ? "" : anthropicApiKey,
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {success && (
          <div className="p-3 text-sm bg-emerald-50 text-emerald-600 rounded-lg border border-emerald-200">
            {success}
          </div>
        )}
              ...
              <div className="flex gap-3">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="accent-primary"
                  />
                  <span className="text-sm">GLM API</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name="agentType"
                    value="claude-sub"
                    checked={agentType === "claude-sub"}
                    onChange={() => setAgentType("claude-sub")}
                    className="accent-primary"
                  />
                  <span className="text-sm">Claude 订阅</span>
                </label>
              </div>
              <p className="text-xs text-muted-foreground">
                {agentType === "claude"
                  ? "使用 Claude API，支持 Claude 系列模型及 LiteLLM 代理"
                  : agentType === "glm"
                  ? "使用智谱 GLM API，原生支持 GLM 系列模型"
                  : "使用 Claude 订阅账号，需在容器终端中执行 claude /login 登录"}
              </p>
            </div>
            {agentType !== "claude-sub" && (
              <>
                <div className="space-y-2">
