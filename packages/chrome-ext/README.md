...
点击侧边栏顶部的绿色连接状态点，可以为当前 `Agent 地址 + 项目` 设置两项授权：

- **始终允许修改**：允许 Agent 在当前项目内执行本次需求及测试失败后的代码修复；不包含发布、部署、Git push、项目外文件和其他高风险操作。
- **始终允许自动测试**：开发完成后自动启动当前页面的真实浏览器测试，不再逐次等待“开始测试”确认。

未开启永久授权时，确认卡片仍提供一次性操作，并额外提供快捷授权按钮：

- 待测试时可选择“开始测试”或“始终允许并开始测试”。
- 测试失败时可选择“修复问题”或“始终允许并修复”。

两项授权同时开启后，Chrome 扩展会执行“开发 → 测试 → 发现代码缺陷 → 修复 → 重测”闭环。每项需求最多进行 3 次测试；环境、数据、权限或证据不足类失败不会触发自动修改。关闭任一授权后，后续自动轮次立即停止。Widget 不提供永久授权或自动测试闭环。
