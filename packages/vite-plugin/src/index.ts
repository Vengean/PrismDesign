...
function findWidgetScript(): string | null {
  // Strategy 1: resolve from prism-studio-widget package
  try {
    const widgetPkg = require.resolve("@prism-studio-ai/widget/package.json");
    const widgetDir = path.dirname(widgetPkg);
    const iifeFile = path.join(widgetDir, "dist", "prism-studio-widget.iife.js");
  ...
  try {
    ...
    const candidate = path.resolve(thisDir, "../../widget/dist/prism-studio-widget.iife.js");
...
export default function prismStudio(options: PrismStudioOptions = {}): Plugin {
  const {
  ...
  return {
    ...
    async configureServer(server) {
      if (widget) {
        ...
        if (widgetPath) {
          ...
          config.logger.warn("[Prism Studio] Widget script not found. Run: pnpm build:widget");
          return;
        }
        ...
        config.logger.info("[Prism Studio] Widget injection disabled; use the Chrome extension to connect to the Agent.");
      }
      ...
      if (agentUrl && !agentUrlOverride) {
        ...
        if (localMatch) {
          ...
          proxyingAgent = true;
          agentUrl = "__PROXY__";
          config.logger.info(`[Prism Studio] Proxying agent via dev server (${localAgentUrl})`);
        }
      }
      ...
      if (!agentUrl && agentAutoStart) {
        ...
        if (agentCli) {
          ...
          const agentArgs = [agentCli, "start", "--port", String(preferredPort), "--project", projectRoot];
            ...
            agentProcess!.stdout?.on("data", (data: Buffer) => {
              ...
              for (const line of text.split("\n")) {
                ...
                if (trimmed && !trimmed.startsWith("__PRISM_AGENT_PORT__") && !trimmed.startsWith("__PRISM_AGENT_TOKEN__")) {
                  config.logger.info(`[Prism Studio Agent] ${trimmed}`);
                }
              }
            });
          });
          ...
          agentProcess.stderr?.on("data", (data: Buffer) => {
            ...
            if (msg) config.logger.warn(`[Prism Studio Agent] ${msg}`);
          });

          agentProcess.on("exit", (code) => {
            if (code !== null && code !== 0) {
              config.logger.error(`[Prism Studio] Agent exited with code ${code}`);
            }
            agentProcess = null;
          });
          ...
          proxyingAgent = true;
          agentUrl = "__PROXY__";
          config.logger.info(`[Prism Studio] Agent ready on port ${actualPort} (proxied via dev server)`);
        } else {
          config.logger.warn("[Prism Studio] Agent CLI not found. Install @prism-studio-ai/agent or run agent manually.");
          config.logger.info("[Prism Studio] Widget will show connection form for manual URL input.");
        }
      } else if (agentUrl) {
        config.logger.info(`[Prism Studio] Using existing agent at ${agentUrl}`);
      }
    ...
    transformIndexHtml() {
      ...
      if (agentUrl === "__PROXY__") {
        ...
        initScript = `(function(){var o=${optionsJson};o.agentUrl=location.origin+"/__prism_agent__";PrismStudioWidget.init(o)})();`;
      } else if (agentUrl.startsWith("__AGENT_PORT__:")) {
        const port = agentUrl.split(":")[1];
        const optionsJson = JSON.stringify(initOptions);
        initScript = `(function(){var o=${optionsJson};o.agentUrl="http://"+location.hostname+":${port}";PrismStudioWidget.init(o)})();`;
      } else if (agentUrl) {
        initOptions.agentUrl = agentUrl;
        initScript = `PrismStudioWidget.init(${JSON.stringify(initOptions)});`;
      } else {
        initScript = `PrismStudioWidget.init(${JSON.stringify(initOptions)});`;
      }
