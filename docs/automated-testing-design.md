...
按钮路径是确定性授权：点击“开始测试”后 Orchestrator 直接启动对应 Verification，但实际浏览器操作仍由 Agent 通过工具完成。Chrome 插件还可以按 `Agent URL + projectRoot` 持久化“始终允许修改”和“始终允许自动测试”；Widget 不支持这两项授权。

未开启永久授权时，待测试卡片提供“始终允许并开始测试”，失败卡片提供“始终允许并修复”。快捷按钮必须先完成本地持久化并同步后台权限状态，再执行当前动作，避免第一次操作仍使用旧授权。
...
3. 用户授权后修改代码（已完成）：测试过程不修改代码；失败卡片提供一次性“修复问题”和“始终允许并修复”，随后把目标、失败用例和建议交给 Agent 开发修复。
4. 等待 HMR/页面重新注册。
5. 修复完成后生成复测项（已完成）；开启自动测试时直接复测，否则等待用户确认。
6. 自动闭环最多执行 3 次测试，仅 `code_defect` 或暂未能细分的 `unknown` 失败可以在已授权修改时进入修复；环境、数据、权限和证据不足类失败立即停止。
